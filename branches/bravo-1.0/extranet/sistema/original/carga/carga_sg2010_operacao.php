<?php
	...
	for ($i=0; $i<$loops; $i++){	
		...
		while($linha_arquivo = fgets ($arquivo)) {
			...
			$sqlPcpOperacao = mysql_query ("SELECT null FROM tb_pcp_operacao WHERE CO_RECNO = '".addslashes (trim ($linha[4]))."'",$conexao)
			or die (mysql_error ());
			...
			if (mysql_num_rows ($sqlPcpOperacao) == 0){
			    mysql_query ("INSERT INTO tb_pcp_operacao (CO_RECURSO
				              ...
				              VALUES ('".addslashes (trim ($linha[0]))."'
								  ...
								  , '".addslashes (trim ($linha[4]))."')",$conexao)
			    or die  (mysql_error ()." ->  (insereOperacao)");
			}
