<?php
    ...
    require_once APP_PATH.'sistema/setup.php';

	/**
	 * Classe generica para grid, modificada dia 05/11/2012 generalizando chamadas, criando setsql e setfrom
	...
	class Paging extends Connection{
		...
		public $s_orderby		= ''; 		//Campo utilizado para ordena��o inicial
		...
		public $_sql			= null;		//Query para listar os dados
		public $_from	        = null;		//From com os joins
		...
		/**
		 * Metodo para setar a query a ser utilizada no grid
		 * @author Ricardo S. Alvarenga
		 * @since 05/1/2012
		...
		/**
		 * Metodo para setar a query a ser utilizada no grid para contabilizar paginação
		 * @author Ricardo S. Alvarenga
		 * @since 05/1/2012
		 * @param String $tabela	from da tabela com os joins
		 */
		public function setCount($from){
			$this->_from = $from;
		}
		...
		public function total_rows(){
			...
			$sth = $this->dbh->prepare('SELECT COUNT(*) FROM '.$this->_from.' WHERE '.$this->s_where);
			$sth->execute();
		...
		public function show_table(){
			...
			//Formula a query			
			$sql = $this->_sql.' WHERE '.$this->s_where.'
			...
			//Se não forem inseridas colunas manualmente, ent�o apenas mostra os resultados do banco
			if($this->a_columns == null){
