<?php
	...
	class Paging extends Connection{
		...
		public $s_orderby		= 'NO_PAPEL'; 	//Campo utilizado para ordena��o inicial
		...
		public function existeModulo($co_papel){
			
			$sth = $this->dbh->prepare("select count(*) from tb_papel_modulo where co_papel =".$co_papel);
			$sth->execute();
			$row = $sth->fetch(PDO::FETCH_NUM);
			return $row;
		}
		...
		public function show_table(){
			...
			$s_html .= '<script type="text/javascript" src="js/cadastros/papel.js"></script>
						<div id="confirmaExcluirPapel"><p>Deseja realmente excluir esse registro?</p></div>
						<div id="formularioAlterarPapel"></div>';
			...
			if($this->a_columns == null){
				...
				while($row = $sth->fetch(PDO::FETCH_NUM)){
					...
					//EXISTE MODULO
					$existeModuloCadastrado = $this->existeModulo($row[0]);
